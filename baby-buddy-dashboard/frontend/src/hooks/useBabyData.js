  ...
  const fetchData = useCallback(async (childId) => {
    try {
      ...
      const [
        ...
        api.getMedication({ child: c, limit: 20, ordering: "-time" }).catch(() => ({})),
        api.getPumping({ child: c, start_min: todayMin, start_max: todayMax, limit: 100, ordering: "-time" }).catch(() => ({})),
        api.getPumping({ child: c, start_min: weekMin, limit: 200, ordering: "-time" }).catch(() => ({})),
        api.getBmi({ child: c, limit: 20, ordering: "-date" }).catch(() => ({})),
        api.getHeadCircumference({ child: c, limit: 20, ordering: "-date" }).catch(() => ({})),
        api.getTimers(),
